...
def theTTP(filename):
    ...
    if list(targets.columns) != ['Starname', 'RA', 'Dec', 'Exposure Time', 'Exposures Per Visit', 'Visits In Night', 'Intra_Night_Cadence', 'Priority', 'Maximum Airmass']:
        print(list(targets.columns))
        print("Error: column names not correct.")
        print("Column names must be this format and this order: ['Starname', 'RA', 'Dec', 'Exposure Time', 'Exposures Per Visit', 'Visits In Night', 'Intra_Night_Cadence', 'Priority', 'Maximum Airmass'].")
        return
    else:
